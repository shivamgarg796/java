...
//			e.printStackTrace();
	...
	@Override
	public String updateExistingStudent(Student student) throws StudentExceptions {
		...
		try(Connection conn=DBUtil.provideConnection()) {
			...
			try {
			Student existingstudent = getStudentByRoll(student.getRoll());
			
			PreparedStatement ps=conn.prepareStatement("update student set name=?,address=?,marks=? where roll=?");
			...
			msg=e.getMessage();
			throw new StudentExceptions(msg);
		}
